...
export default function AddGuest(props) {
  ...
  const[adLimit,setAdLimit] = useState(3);
  const[kidLimit,setKidLimit] = useState(1);
  ...
  const postResults = () => {
    ...
    dispatch(actionCreators.addAdult(ad));
    dispatch(actionCreators.addKid(kids));
    dispatch(actionCreators.addRoom(rows.length));
    console.log(days);
    dispatch(actionCreators.addDays(days,price,rows.length));
  ...
  const updateState = (e) => {
    ...
    if(e.target.attributes.column.value === 'Kids'){
      setAdLimit(3 - e.target.value);
    }
    else{
      setKidLimit(3-e.target.value);
    }
          ...
          <div className="col-md-12 column">
            <table className="table table-bordered table-hover" id="tab_logic">
              ...
              <tbody>
                {rows.map((item, idx) => (
                  <tr key={idx}>
                    <td className="text-center">Room #{idx + 1}</td>
            ...
            <div className="text-center">You have selected {rows.length} rooms.</div><br/><br/>
